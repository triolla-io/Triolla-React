...
import { GET_CYBER_SECURITY_PAGE, GET_THEME_SETTINGS } from "@/lib/queries";
...
async function getCyberSecurityData() {
  try {
    const { data } = await client.query<any>({
      query: gql`${GET_CYBER_SECURITY_PAGE}`,
    });
    return data?.page?.template?.portfolioFields ?? {};
  } catch {
    return {};
  }
}
...
export default async function CyberSecurityPage() {
  const [pf, ts] = await Promise.all([getCyberSecurityData(), getThemeSettings()]);
