      ...
      <AnimatePresence>
        {open && item.children.length > 0 && (
          <motion.div
            ...
            className="absolute top-full left-1/2 -translate-x-1/2 mt-4 bg-white rounded-2xl shadow-2xl shadow-black/15 p-6 z-50"
            style={{ minWidth: Math.min(item.children.length, 6) > 3 ? 460 : 240 }}
          >
            <div
  ...
  return (
    <header className="sticky top-0 z-50 w-full pointer-events-none">
      ...
      <div className="pointer-events-auto flex justify-center px-4 pt-4">
        <div className="relative flex items-center gap-3 bg-[#0a0a0a] text-white rounded-full border border-white/10 shadow-2xl shadow-black/40 w-full max-w-[920px] px-5 h-[54px]">
          ...
          <div className="hidden lg:flex items-center gap-2.5 ml-auto">
            {contactButtonHref && contactButtonText && (
              <Link
                ...
                className="bg-yellow-400 text-black rounded-full px-5 py-[7px] text-[13px] font-semibold hover:bg-yellow-300 transition-colors whitespace-nowrap"
              >
                {contactButtonText}
              </Link>
            )}
