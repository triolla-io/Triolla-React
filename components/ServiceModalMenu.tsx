...
/** One Product service, enriched server-side from its WP detail page.
...
export interface ServiceDetail {
  label: string | null; // prodmtitle — the menu label
  link: string | null; // prodmlink — fallback href
        ...
        {services.map((s, i) => (
          <FadeIn key={i} delay={i * 0.09} yOffset={18}>
            <li className="svc-menu-item">
              <span className="svc-menu-item__num">{num(i)}</span>
              {s.hasDetail ? (
                <button
                  type="button"
                  ref={(el) => {
                  ...
                  className="svc-menu-item__title svc-menu-item__title--btn"
                  onClick={() => open(i)}
                ...
                <a href={s.link} className="svc-menu-item__title">
                  {s.label}
                </a>
              ) : (
                <span className="svc-menu-item__title">{s.label}</span>
              )}
            </li>
          </FadeIn>
        ))}
      ...
      {isOpen &&
        ...
        createPortal(
          ...
          >
            ...
            <style>{`
              /* Menu trigger buttons reuse .svc-menu-item__title verbatim —
                 reset only the native button box so the list looks identical
                 to the original <a>/<span> rendering (the class keeps
                 display:block, font, color, etc.). */
