 ...
 */
export function useServiceModal(services: ServiceDetail[], { ctaText, ctaLink }: UseServiceModalOptions = {}) {
  ...
  const goRelative = useCallback(
    (dir: 1 | -1) => {
      ...
      const nextPos = (pos + dir + detailIndices.length) % detailIndices.length
      const nextIndex = detailIndices[nextPos]
      lastIndexRef.current = nextIndex
      setActiveIndex(nextIndex)
      // Reset scroll + parallax for the incoming service.
      if (scrollRef.current) scrollRef.current.scrollTop = 0
      if (heroMediaRef.current) heroMediaRef.current.style.transform = 'translate3d(0,0,0)'
    },
          ...
          >
            ...
            <div className="svcm-panel">
              ...
              <div className="svcm-bar">
                ...
                {active.label && <span className="svcm-bar__eyebrow">{active.label}</span>}
                <button type="button" ref={closeBtnRef} className="svcm-close" onClick={close} aria-label="Close">
                  <span aria-hidden="true">✕</span>
                </button>
              </div>

              {/* Scrollable content — keyed so reveals replay on prev/next */}
              <div className="svcm-scroll" ref={scrollRef} onScroll={onScroll} key={activeIndex}>
                {/* Hero stage */}
                {active.image && (
                  <div className="svcm-hero">
                    <div className="svcm-hero__media" ref={heroMediaRef}>
                      <img src={active.image} alt={active.altText} className="svcm-hero__img" />
                    </div>
                ...
                <div className="svcm-article">
                  {active.title && (
                    <h2 className="svcm-title" style={{ animationDelay: '0.12s' }}>
                      {active.title}
                    </h2>
                  )}
                  ...
                  <div className="svcm-footer">
                    {detailIndices.length > 1 && (
                      <div className="svcm-nav">
                        <button type="button" className="svcm-nav__btn" onClick={() => goRelative(-1)}>
